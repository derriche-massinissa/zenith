...
{
	auto [scale, textured] = g_registry.try_get<Components::Scale, Components::Textured>(entity);
...
{
	auto [size, textured] = g_registry.try_get<Components::Size, Components::Textured>(entity);
	...
	if (frame == entt::null)
	{
		fr = g_registry.try_get<Components::Frame>(textured->frame);
	}
	else
	{
		fr = g_registry.try_get<Components::Frame>(frame);
	}
...
{
	auto [size, update] = g_registry.try_get<Components::Size, Components::Update<Components::Size>>(entity);
