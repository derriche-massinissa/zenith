 ...
 * @param key The unique key of this Texture.
 * @param sources A vector of paths to the files that are used to create
 * the texture.
 * @param width The width of the Texture. This is optional and automatically 
 * derived from the source images.
 * @param height The height of the Texture. This is optional and automatically 
 * derived from the source images.
 */
class Texture
{
public:
	Texture(TextureManager& manager, std::string key, std::vector<std::string> sources);
	~Texture();
     ...
     */
    Frame* add (std::string name, int sourceIndex, int x, int y, int width, int height);
     ...
     */
    int getTextureSourceIndex (TextureSource& source);
     ...
     */
	std::vector<Frame*> getFramesFromTextureSource (int sourceIndex, bool includeBase = false);
     ...
     */
	std::vector<std::string> getFrameNames (bool includeBase = false);

    /**
     * Given a Frame name, return the source image it uses to render with, in
	 * the form of a RenderTexture game object.
     *
     * @since 0.0.0
     *
     ...
     */
	GameObjects::RenderTexture getSourceImage (std::string name);
	 ...
	 * @since 0.0.0
	 */
	std::vector<TextureSource> source_;
